#!/bin/bash
# SAT 批量测试脚本 v3：三引擎 (基础版/2WL+JW/CDCL) 新求解器
# exe: build/Release, 每引擎内部 10s 上限, 外壳 120s 保险
ROOT="d:/program/2026秋-程序设计综合课程设计任务及指导学生包"
EXE="$ROOT/SAT_solver/build/Release/sat_solver.exe"
LOGDIR="$ROOT/SAT_solver/test_logs3"
mkdir -p "$LOGDIR"
...
run_one() {
    ...
    start=$(date +%s)
    timeout 120 "$EXE" "$cnf" > "$LOGDIR/$tag.log" 2>&1
    code=$?
    end=$(date +%s)
    wall=$((end - start))
