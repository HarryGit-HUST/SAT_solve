...
run_one() {
    ...
    start=$(date +%s)
    timeout "$to" "$EXE" "$cnf" > "$LOGDIR/$tag.log" 2>&1
    code=$?
    end=$(date +%s)
    wall=$((end - start))
...
echo "===== 小型算例 ====="
for c in "${SMALL[@]}"; do run_one "$c" 300; done

echo "===== 中型算例 ====="
for c in "${MEDIUM[@]}"; do run_one "$c" 300; done

echo "===== 大型算例 ====="
for c in "${LARGE[@]}"; do run_one "$c" 300; done

echo "===== 极难算例(600s) ====="
for c in "${HARD[@]}"; do run_one "$c" 600; done
